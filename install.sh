...
if [ ! -n "$BASH" ]; then
	...
	echo "Non-bash shell detected, fixing..."	bash -c '. <('"$(command -v curl >/dev/null && echo 'curl -Ls' || echo 'wget -qO-')"' https://github.com/GeekTG/Friendly-Telegram/raw/master/install.sh) '"$*"
